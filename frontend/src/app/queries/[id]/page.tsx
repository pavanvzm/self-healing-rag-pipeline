...
import {
  ...
  Brain, FileText, Shield, Clock, ChevronDown, ChevronUp,
  ArrowRight,
} from "lucide-react";
      ...
      <div className="space-y-1">
        {logs.map((log, idx) => {
          ...
          return (
            <div
              key={idx}
              className="group cursor-pointer rounded-lg border border-surface-800/50 p-3 transition-colors hover:border-surface-700 hover:bg-surface-800/20"
              onClick={() => setExpandedLog(isExpanded ? null : idx)}
            >
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-3 min-w-0">
                  <div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-md ${
                    log.score !== null && log.score >= 0.8
                      ? "bg-success/10 text-success"
                      : log.score !== null && log.score >= 0.6
                  ...
                  <div className="min-w-0">
                    <p className="truncate text-sm text-surface-200 group-hover:text-surface-100">
                      {log.message}
                    </p>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="card p-8 text-center">
          ...
          <p className="text-sm text-surface-400 mb-4">The query with ID "{id}" could not be found.</p>
    ...
    <div className="min-h-screen bg-surface-950">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex h-16 items-center justify-between">
            <div className="flex items-center gap-4">
              <Link
                href="/"
                className="flex items-center gap-2 rounded-lg px-3 py-2 text-sm text-surface-400 transition-colors hover:bg-surface-800 hover:text-surface-200"
              >
      ...
      <main className="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8 space-y-6 animate-fade-in">
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-3">
            <div className="rounded-lg bg-surface-800/30 p-4">
              ...
              <p className={`text-2xl font-bold ${
                query.confidence >= 0.8 ? "text-success" : query.confidence >= 0.6 ? "text-warning" : "text-danger"
              }`}>
            ...
            <div className="rounded-lg bg-surface-800/30 p-4">
              ...
              <p className="text-2xl font-bold text-warning">
                {query.healing_actions}
                <span className="text-sm font-normal text-surface-400 ml-1">attempts</span>
              </p>
            </div>
            <div className="rounded-lg bg-surface-800/30 p-4">
        ...
        <div className="text-center pt-4">
          <Link
            href="/"
            className="inline-flex items-center gap-2 rounded-lg bg-brand-600 px-5 py-2.5 text-sm font-medium text-white transition-all hover:bg-brand-500 shadow-lg shadow-brand-500/20"
          >
            <ArrowLeft className="h-4 w-4" />
            Back to Dashboard
          </Link>
